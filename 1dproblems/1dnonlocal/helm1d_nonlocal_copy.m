...
lhs(1:N_int,1:N_int) = eye(N_int) - beta*Gs_int - gamma*Gphi_int ...
    - S_int + beta*S_tot(N_left+1:N_left+N_int,:)*Gs_tot(:,N_left+1:N_left+N_int) ...
    + gamma*S_tot(N_left+1:N_left+N_int,:)*Gphi_tot(:,N_left+1:N_left+N_int);

lhs(1:N_int,N_int+1) = gskern(bdry1,int_chnkr) - S_tot(N_left+1:N_left+N_int,:)*gskern(bdry1,tot_chnkr);
lhs(1:N_int,N_int+2) = gskern(bdry2,int_chnkr) - S_tot(N_left+1:N_left+N_int,:)*gskern(bdry2,tot_chnkr);
...
rhs = [- Gs_tot(N_left+1:N_left+N_int,1:N_left)*f + S_tot(N_left+1:N_left+N_int,:)*Gs_tot(:,1:N_left)*f ; -grad2*(f.*wts(:)); -grad3*(f.*wts(:))]; 
...
usolkern = @(s,t) delgkern(rts,ejs,s,t);

opts = [];
opts.sing = 'log';

lapGphimat = chunkermat(tot_chnkr,usolkern,opts);
Susol = lapGphimat*mu + Gphi_tot(:,1:N_left)*f+ mu1*gphikern(bdry1,tot_chnkr) + mu2*gphikern(bdry2,tot_chnkr);
...
int_err = uint - S_tot(N_left+1:N_left+N_int,:)*usol;
...
int_err2 = uint - Susol(N_left+1:N_left+N_int);
int_err2 = int_err2 ./ max(abs(uint));

figure(20);
plot(int_chnkr.r(1,:), real(int_err2), int_chnkr.r(1,:), imag(int_err2))
