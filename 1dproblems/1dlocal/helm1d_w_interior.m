...
[~,grad] = helm1d.green(k,src1,bdry1);
...
uleft = gkern(src1,left_chnkr) + mu1*gkern(bdry1,left_chnkr);
...
rhs = [- gkern(src1,int_chnkr) + S(N_left+1:N_left+N_int,:)*gkern(src1,tot_chnkr) ; -grad2; -grad3; 0; 0];
...
% method 3, solve preconditioned integral equation
...
S_tot = chunkermat(tot_chnkr,skern,opts);
S_int = chunkermat(int_chnkr,skern,opts);
...
[~,grad,~,third] = gkern(int_chnkr,bdry1);
...
[~,grad,~,third] = gkern(int_chnkr,bdry2);
...
rhs = [-gkern(src1,int_chnkr) + S_tot(N_left+1:N_left+N_int,:)*gkern(src1,tot_chnkr); -grad2; -grad3];
...
usol = mu - k^2*G_tot*mu + gkern(src1,tot_chnkr) + mu1*gkern(bdry1,tot_chnkr) + mu2*gkern(bdry2,tot_chnkr);
