...
for n = -N/4:N/4
    ...
    for i = 1:16
        R2 = R+rs(i);
        k11s = [k11s; integral(@(t) exp(1i*n*t).*(K11(rts,ejs,zk,nu,R,R2,t)-1i*sign(n)*K11H(rts,ejs,zk,nu,R,R2,t)),0,2*pi)];
        k12s = [k12s; integral(@(t) exp(1i*n*t).*K12(rts,ejs,zk,nu,R,R2,t),0,2*pi)];
        k21s = [k21s; integral(@(t) exp(1i*n*t).*(K21(rts,ejs,zk,nu,R,R2,t)-1i*sign(n)*K21H(rts,ejs,zk,nu,R,R2,t)),0,2*pi, "AbsTol",1E-8,"RelTol",1E-8)];
        k22s = [k22s; integral(@(t) exp(1i*n*t).*K22(rts,ejs,zk,nu,R,R2,t),0,2*pi)];
    end
    ...
    bc1 = integral(@(t) exp(1i*n*t).*BC1(rts,ejs,zk,nu,sqrt(src(1)^2+src(2)^2),R,t).',0,2*pi);
    bc2 = integral(@(t) exp(1i*n*t).*BC2(rts,ejs,zk,nu,sqrt(src(1)^2+src(2)^2),R,t).',0,2*pi);
...
[val,grad] = flex2d.fggreen(srcs,targs,rts,ejs);
...
[G0,~] = flex2d.fggreen(src,targs,rts,ejs);
...
function val = K11(rts,ejs,zk,nu,R1,rs,t)
    
    src = R1*[cos(t); sin(t)];
    targ = [rs 0*rs].';

    [~,~,~,third] = flex2d.fggreen(src,targ,rts,ejs);
...
function val = K11H(rts,ejs,zk,nu,R1,rs,t)
...
function val = K12(rts,ejs,zk,nu,R1,rs,t)
    
    src = R1*[cos(t); sin(t)];
    targ = [rs 0*rs].';
    [~,~,hess] = flex2d.fggreen(src,targ,rts,ejs);
...
function val = K21(rts,ejs,zk,nu,R1,rs,t)
    
    src = R1*[cos(t); sin(t)];
    targ = [rs 0*rs].';
    [~,~,~,third,forth] = flex2d.fggreen(src,targ,rts,ejs);
...
function val = K21H(rts,ejs,zk,nu,R1,rs,t)
...
function val = K22(rts,ejs,zk,nu,R1,rs,t)
    
    src = R1*[cos(t); sin(t)];
    targ = [rs 0*rs].';
    [~,~,hess,third] = flex2d.fggreen(src,targ,rts,ejs);
...
function val = BC1(rts,ejs,zk,nu,R1,rs,t)
...
function val = BC2(rts,ejs,zk,nu,R1,rs,t)
