function [phi,gradphi] = gphi(src,targ,rts,ejs,ifr2logr)
%
...
for i = 1:5
    ...
    if (angle(rhoj) == 0) && (rhoj ~= 0)
        ...
        if nargout == 1
           [sk0] = flex2d.struveKdiffgreen(rhoj,src,targ,ifr2logr);
           [h0] = flex2d.helmdiffgreen(rhoj,src,targ,ifr2logr);
        elseif nargout == 2
           [sk0,gradsk0] = flex2d.struveKdiffgreen(rhoj,src,targ,ifr2logr);
           [h0,gradh0] = flex2d.helmdiffgreen(rhoj,src,targ,ifr2logr);
        end
        % elseif nargout == 3
        %    [sk0,gradsk0,hesssk0] = flex2d.struveKdiffgreen(rhoj,src,targ,ifr2logr);
        %    [h0,gradh0,hessh0] = flex2d.helmdiffgreen(rhoj,src,targ,ifr2logr);
        % elseif nargout == 4
        %    [sk0,gradsk0,hesssk0,thirdsk0] = flex2d.struveKdiffgreen(rhoj,src,targ,ifr2logr);
...
if nargout > 1
gradphi(:,:,1) = 1/2*gradphix;
gradphi(:,:,2) = 1/2*gradphiy;
end
